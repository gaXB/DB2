...
** output: none
****************************************************************/
status_t erase_flash_except_bootloader(void)
{
	status_t ret = STATUS_SUCCESS;

	/*disable CPU global interrupt to avoid RWW violation*/
	INT_SYS_DisableIRQGlobal();

	/*start flash sector erase*/
	ret = FLASH_DRV_EraseSector(&flashSSDConfig, 0x40000, (flashSSDConfig.PFlashSize- 0x40000));

	/*re-enable CPU global interrupt */
...
status_t Download_Data_to_Flash(uint32_t flash_address,const uint8_t* pData, uint16_t data_size)
{
	...
	/* program the S19 data */
	ret = FLASH_DRV_Program(&flashSSDConfig,flash_address,data_size,pData);
...
{
	status_t ret;
	ret = FLASH_DRV_EEEWrite(&flashSSDConfig, flash_address + DATAFLASH_BASE_ADDR, data_size, pData);
...
/*******************************************************************************
 * Function:       inline void FLASH_EraseSector(uint32 fAddr)
 *
 * Description:     codeflash擦除
 *
 * Returns:         none
 *
 * Notes:           	fAddr ，4字节对齐
...
void FLASH_EraseSector(uint32 fAddr)
{
	...
	/*start flash sector erase*/
	(void)FLASH_DRV_EraseSector(&flashSSDConfig, fAddr, 0x1000);
...
void GetFlash_CRC32(uint32 eeAddr, uint32 size, uint8* CRCData)
{
	uint32 crcValue = 0xFFFFFFFF;
	uint8* fAddr = (uint8*)eeAddr;
	int i;
	for (i = 0; i<size ; i++)
	{
		crcValue = (crcValue << 8) ^ CRC32TABLE[((crcValue >>24 ) & 0xFF) ^ (*fAddr++)];
	}
	crcValue =~crcValue;

	CRCData[0] = (uint8)(crcValue >> 24);
	CRCData[1] = (uint8)(crcValue >> 16);
	CRCData[2] = (uint8)(crcValue >> 8);
	CRCData[3] = (uint8)(crcValue);
}
