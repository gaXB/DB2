...
#define   O_CANEN   GPIOA_PTD2     //如果没有CANWAKE 功能则设置为GPIO_UNUSE,也可以更改CANDrive_SleepIO
#define   CANWAKE_KBI_NUM       0    // kbi 的号及通道，如无此功能，可随意定义
#define   CANWAKE_KBI_CHANEL    27

/*************以下不能更改*************************************************/
//can 错误和接收中断控制宏定义
#define EnableCanErrTxInterrupt()      NOP()//NVIC_DisableIRQ(MSCAN_TX_IRQn)
#define DisableCanErrTxInterrupt()    NOP() //NVIC_EnableIRQ(MSCAN_TX_IRQn)
#define DisableCanRxInterrupt()		NOP()//	CAN_ReceiverFullIntDisable(MSCAN)
#define EnableCanRxInterrupt()       NOP()//  CAN_ReceiverFullIntEn(MSCAN)
