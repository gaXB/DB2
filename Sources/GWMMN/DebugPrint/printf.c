...
#if (DEBUG_PRINT_ == 1)
#define   MAXPRINT_LENGTH     256   //8的倍数
/********************************************************************/
char u8PrintBuffer[MAXPRINT_LENGTH];
typedef struct
{
    int dest;
    void (*func)(char);
    ...
    switch (info->dest)
    {
        case DEST_CONSOLE:
           // info->func((char)c);
            break;
        case DEST_STRING:
            if (info->ilength < MAXPRINT_LENGTH)
            {
...
printf (const char *fmt, ...)  //结尾加\n
{
   ...
   info.ilength = 0; 
	...
	va_end(ap);
	out_char(u8PrintBuffer, info.ilength);
   return rvalue;
}
...
void out_char(char *ch, int length)
{
	...
	do
	{
		...
		while (CANDrive_TxBufferEmptemp() == FALSE)
		{
			u32Wait++;
			if (u32Wait > 0x2000)
			{
	   ...
	   (void)CANDrive_SendFram(DEBUG_ID, pSend);
	   pSend += 8;
	}while(length > 0);
}
#endif
